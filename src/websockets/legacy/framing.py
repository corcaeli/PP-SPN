...
class Frame(NewFrame):
    MEASUREMENT_total_message_lengths = 0
    MEASUREMENT_total_message_amount = 0
    ...
    async def read(
        ...
        *,
        ...
        # Read the header.
        data = await reader(2)
        MEASUREMENT_current_message_length = len(data)
        ...
        MEASUREMENT_current_message_length += len(data)
        Frame.MEASUREMENT_total_message_lengths += MEASUREMENT_current_message_length
        if mask:
        ...
        frame.check()
        Frame.MEASUREMENT_total_message_amount += 1
        return frame
