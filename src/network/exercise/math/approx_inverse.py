...
def add_exercise_approx_inverse_new(manager, data_id, data_id_result=None):
    # appinv(x) = 2^n / x
    ...
    #data_id_double_id = f"{data_id}_approx_inverse_double_id"
    #add_exercise_addition(manager, data_id, data_id, data_id_double_id)                                     # p+p

    data_id_two_pow_n_plus_1= f"{data_id}_approx_inverse_two_pow_n_plus_1"
    manager.data[data_id_two_pow_n_plus_1] = pow(2,n+1)
    add_exercise_manager_inser_in_share(manager, data_id_two_pow_n_plus_1)                                  # pow(2,n+1)

    #data_id_y_dummy = f"{data_id}_approx_inverse_y_dummy"
    #add_exercise_addition(manager, data_id_two_pow_n_plus_1, data_id, data_id_y_dummy)            # y_dummy = pow(2,n+1) + (p+p)

    data_id_y = f"{data_id}_approx_inverse_y"
    data_id_x = f"{data_id}_approx_inverse_x"
    data_id_w = f"{data_id}_approx_inverse_w"

    data_id_two_pow_t_minus_1= f"{data_id}_approx_inverse_two_pow_t_minus_111"
    manager.data[data_id_two_pow_t_minus_1] = pow(2,t-1)
    add_exercise_manager_inser_in_share(manager, data_id_two_pow_t_minus_1)

    manager.data[data_id_u] = 3 * pow(2, t - 2)                                                             # u = 3* pow(2,t-2)
    ...
    while i <= amount_rounds + n:
        add_exercise_multiplication(manager, data_id, data_id_u, data_id_tmp)                               # tmp = p * u
        add_exercise_trunc(manager, data_id_tmp, pow(2,t), data_id_z)                                       # z = trunc(tmp, 2^t)


        #add_exercise_subtraction(manager, DataIDs.APPINVERSE_OF_TWO_POW_N_PLUS_1, data_id_z, data_id_y)     # y = pow(2, n + 1) - z
        add_exercise_subtraction(manager, data_id_two_pow_n_plus_1, data_id_z, data_id_y)                            # y = y_dummy - z = pow(2,n+1) + (p+p) - z


        add_exercise_multiplication(manager, data_id_u, data_id_y, data_id_x)                               # x = u * y
        add_exercise_trunc(manager, data_id_x, pow(2,n), data_id_u)                                                # u = trunc(x, n)
        i = i + 1
    ...
    add_exercise_addition(manager, data_id_u, data_id_two_pow_t_minus_1, data_id_w)                         # w = u + pow(2, t - 1)
    add_exercise_trunc(manager, data_id_w, pow(2,t), data_id_result)                                        # u = trunc(w, t)
...
def add_exercise_approx_inverse(manager, data_id, data_id_result=None):
    ...
    while i <= amount_rounds + n:
        ...
        add_exercise_trunc(manager, data_id_tmp, pow(2,t), data_id_z)  # z = trunc(tmp, t)
