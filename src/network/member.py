...
from network.id_chip import IDChip
...
class Member:
    ...
    @property
    def joint_random_zero_additive_minimum(self):
        return self.__joint_random_zero_additive_minimum
    ...
    @property
    def additive_sharing_over_integer_max_secret(self):
        return self.__additive_sharing_over_integer_max_secret

    @property
    def additive_sharing_over_integer_allowed_bit_length_of_secret(self):
        return self.__additive_sharing_over_integer_allowed_bit_length_of_secret

    @property
    def additive_sharing_over_integer_security_parameter(self):
        return self.__additive_sharing_over_integer_security_parameter
    ...
    def load_config(self, configuration_filepath) -> None:
        ...
        self.__joint_random_zero_additive_minimum = int(
            general_section.get(Keys.CONFIG_GENERAL_JOIN_RANDOM_ZERO_ADDITIVE_MINUMUM)
        )
        ...
        self.__additive_sharing_over_integer_max_secret = int(
            general_section.get(
                Keys.CONFIG_GENERAL_ADDITIVE_SHARING_OVER_INTEGER_MAX_SECRET
            )
        )

        self.__additive_sharing_over_integer_security_parameter = int(
            general_section.get(
                Keys.CONFIG_GENERAL_ADDITIVE_SHARING_OVER_INTEGER_SECURITY_PARAMETER
            )
        )

        self.__additive_sharing_over_integer_allowed_bit_length_of_secret = int(
            general_section.get(
                Keys.CONFIG_GENERAL_ADDITIVE_SHARING_OVER_INTEGER_ALLOWED_BIT_LENGTH_OF_SECRET
            )
        )
    ...
    def insert_in_share_additive(self, data_id, value):
        ...
        for index in range(0, amount_parts - 1):
            ...
            tmp = randint(self.joint_random_zero_additive_minimum, values[index])
    ...
    def insert_in_share_additive_integer(self, data_id, value):
        ...
        upper_bound = self.additive_sharing_over_integer_max_secret * pow(
            2, self.additive_sharing_over_integer_security_parameter
        )
    ...
    def addTo(self, data_id_result, data_id):
        self.data[data_id_result] = (
            self.data.get(data_id_result) + self.data.get(data_id)
        ) % self.prim_number

    ################### hooks ###################
    def build_exercise_hooks(self):

        from network.exercise.insert_edge_usage import insert_edge_usage
        from network.exercise.compute_weight_of_sum_nodes import (
            compute_weight_of_sum_nodes,
        )

        from network.exercise.evaluation.evaluation_for_member import (
            evaluation_single_member_leaf_step,
            evaluation_send_to_single_member_step,
            evaluation_single_member_join_step,
        )

        from network.exercise.math.addition import addition
        from network.exercise.math.subtraction import subtraction
        from network.exercise.math.multiplication import (
        ...
        from network.exercise.math.trunc import trunc_init
        from network.exercise.math.trunc import trunc_h_step
        from network.exercise.math.modulo import modulo_cheating

        from network.exercise.io.load_privat_data_for_private_evaluation_from_file import (
            load_private_data_for_private_evaluation_from_file,
        )
        from network.exercise.io.load_private_data_from_file import (
            load_private_data_from_file,
        )
        from network.exercise.io.load_spn_from_file import load_spn_from_file
        from network.exercise.io.load_spn_weights_from_file import (
        ...
        from network.exercise.conversion.si2sq import si2sq
        from network.exercise.conversion.sq2si import (
            sq2si_sharing_step,
            sq2si_result_step,
        )
        from network.exercise.conversion.sq2pq import (
            sq2pq_resharing_step,
            sq2pq_result_step,
        )
        from network.exercise.conversion.pq2sq import (
            pq2sq_resharing_step,
            pq2sq_result_step,
        )

        from network.exercise.joint_random_sharing_of_zero.jriz import (
            jriz_sharing_step,
            jriz_result_step,
        )
        from network.exercise.joint_random_sharing_of_zero.jrpz import (
            jrpz_sharing_step,
            jrpz_result_step,
        )
        from network.exercise.joint_random_sharing_of_zero.jrsz import (
            jrsz_sharing_step,
            jrsz_result_step,
        )

        from network.exercise.reveal.reveal_number import reveal_number
        from network.exercise.reveal.reveal_number_additive import (
            reveal_number_additive,
        )
        from network.exercise.reveal.reveal_number_additive_integer import (
        ...
        from network.exercise.util.manager_inser_in_share import manager_insert_in_share
        from network.exercise.util.delete_data_at_ids import delete_data_at_id
        from network.exercise.util.copy_data import copy_data
        from network.exercise.util.dummy import dummy
